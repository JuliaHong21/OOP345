...
namespace sict {
	...
	CustomerOrder::CustomerOrder(const std::string& str) {  //A one-argument constructor 
		...
		field = util.getFieldWidth();  //field width
	}
	...
	std::string CustomerOrder::getNameProduct() const {  //returns the name of the customer and their product 
		std::string tmp = custName;
		for (size_t i = custName.size(); i < field; i++) {
			tmp += " ";
		}
		return tmp + " [" + std::string(proName) + "]";  //CUSTOMER [PRODUCT]
	}

	void CustomerOrder::display(std::ostream& os, bool showDetail) const {
		os << getNameProduct() << std::endl;  //CUSTOMER [PRODUCT]
		for (size_t i = 0; i < itemNum; i++) {
			if (!showDetail) {
				os << std::setw(field + 1) << " " << itemInfo[i].itemName << std::endl;
			}
			else {
				os << std::setw(field + 1) << " " << "[" << itemInfo[i].serialNum << "]"; // [SERIAL NUMBER]
				os << itemInfo[i].itemName << " -";  //ITEM -
				os << (itemInfo[i].status == true ? "Filled" : "UnFilled") << std::endl;  //FILL_STATUS
			}
		}
	}
}